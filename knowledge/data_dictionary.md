...
>
> 用途：定义**数据源层**的标准字段。所有外部数据源（baostock、AKShare、未来接入的其他 API）必须映射到本字典的字段。下表"标准字段"列即 provider 规范化后的统一字段名。
>
...
**用途**: 主力资金动向分析（免费模式由 AKShare 提供，近约 120 日）。
...
| 数据源 | 模式 | 实现类 | 支持实体 |
|--------|------|--------|---------|
| **免费数据源** | `free` | `CompositeFreeProvider`（baostock + AKShare） | DailyBar, MoneyFlow, StockBasic, DailyBasic(估值) |
...
```
| 标准字段名 | baostock 字段 | AKShare 字段 | 未来数据源字段 |
|-----------|--------------|-------------|---------------|
| close     | close        | 收盘         | xxx           |
```
